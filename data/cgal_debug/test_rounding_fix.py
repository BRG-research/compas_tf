"""Test: what rounding precision fixes the cutter boolean?"""
...
# Rebuild the first failing wedge
edge_contacts = []
...
print("=== Baseline (dirty): ===")
V, F = boolean_difference_mesh_mesh((tv, tt), (cv, ct))
print(f"  V={V.shape[0]} F={F.shape[0]}")

# Test different rounding precisions on the CUTTER
for decimals in [3, 6, 9, 12]:
    cv_rounded = [[round(c, decimals) for c in v] for v in cv]
    V, F = boolean_difference_mesh_mesh((tv, tt), (cv_rounded, ct))
    print(f"  Cutter rounded to {decimals} decimals: V={V.shape[0]} F={F.shape[0]}")

# Test rounding BOTH
for decimals in [3, 6, 9, 12]:
    tv_rounded = [[round(c, decimals) for c in v] for v in tv]
    cv_rounded = [[round(c, decimals) for c in v] for v in cv]
    V, F = boolean_difference_mesh_mesh((tv_rounded, tt), (cv_rounded, ct))
    print(f"  Both rounded to {decimals} decimals: V={V.shape[0]} F={F.shape[0]}")
