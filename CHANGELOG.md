...
* Added six assembly steps to `docs/assembly.md` and the scripts behind them, so the page carries the whole sequence rather than its first two steps: `example_model_23_assembly_step1.py` (the shop - a quarter frame upside down in a jig with the oculus beside it), `_step4` (columns set down on their bases), `_step5` (raking props and the centre tower), `_step6` (the first quarter, with the column steel plates and dowels), `_step7` (the other three quarters and the outer-rib connectors), `_step8` (the oculus) and `_step9` (the wedges and their screws). Each ghosts what is already standing and draws solid what it adds, and each prints the fit it claims - the eight prop clamps meet the 220 shaft at 0.000 mm, the tower's four legs come out on a 1500 x 1520 mm rectangle whose diagonals agree to 0.000 mm, every quarter lands on its column over 11 faces and 0.277 m2 at 0.000 mm, and all 32 wedge screws are level to 0.000 mm.

* Added `examples/assembly_shoring.py`, the prop and tower layout two steps need: step 2 stakes out the tower's legs and step 5 stands the props and the tower on them. It follows `example_model_10_shoring.py` - two props per column at right angles, clamped at 2500 and reaching 2500 along the slab - and exists so the marks and the parts cannot drift apart. A number typed twice is a number that drifts.

* Added `.side-by-side` and `.viewer-bay` to `docs/assets/extra.css`. The first puts an image and its text in two columns and stacks them below 60em; the second gives a whole-bay viewer a taller, squarer box, which is the only thing that actually zooms one in - Online 3D Viewer fits the model's bounding sphere on load, so a `camera` attribute sets the direction and the up vector and its distance is thrown away.

* Added `.marked-photo` and the marks on the SHERPA photograph in step 2: eight dots positioned in per cent of the image, so they scale with it, in the BLUE and PINK of the survey preview beside it. The positions were measured off the image rather than eyed in - the four blue ones are the darkest-blob centroids of the bolt holes, the four pink ones the extremes of the plate's own silhouette against the wood.

* Added `docs/_images/quarter_and_oculus_in_jig.jpg`, the shop photograph of a quarter and an oculus element in their trays, beside step 1's preview of the same two pieces.
...
* Changed `SchoringElement.from_points_and_vectors`, `from_points_and_vectors_no_foot_no_head` and `polylines_to_models` to build a `TFModel` instead of a plain `compas_model.models.Model`. `duplicate()` - re-clone every element with a fresh guid so the copies can be merged alongside the original - is defined on this project's `BaseModel`, not upstream, so `example_model_10_shoring.py` died on `'Model' object has no attribute 'duplicate'` and took `example_model_11_full.py` with it (it reads `shoring_model.json`). These were the last plain-`Model` constructions in `src/`.
* Changed `FloorGuide.bay_height` back to 3500. It went 3500 -> 3000 in `a9fb6f3` "column fab wip" (2026-06-24), one line inside a 25-file commit, and every column has been 500 mm short since: `example_model_2_column_model.py` builds the column as `bay_height - SupportElement.HEIGHT`, so they ran `z = 150 .. 3000` instead of `150 .. 3500`. The whole chain is regenerated from it - floorguide, column, columns, quarters, oculus, floor, cantilever(s), the baked model and its STEP, the shoring and full models, the fabrication files and all nine assembly previews. `story_height = 3200` in `example_model_10_shoring.py` was tuned to the 3500 bay and is right again; it is still hardcoded there and still does not read `guide.bay_height`.

* Changed the assembly series to nine numbered steps, shifting the existing ones by one to make room for the shop step at the front: the survey is step 2, the head plates 3, the columns 4. Scripts, `STEP` constants, preview and fragment filenames, page headings, `_models/` paths, snippet paths, the `#step-N-...` anchors and the cross-references inside the scripts' own docstrings all moved together, so any link into `docs/assembly.md` from outside the repo is off by one.

* Changed `write_colored_obj` to triangulate every face by ear clipping in the face's own plane, instead of writing the tessellation's polygons and leaving them to the viewer. Viewers fan an n-gon from its first vertex, which is correct only for a convex one, and 64 of the 2028 polygon faces in this model are concave - the faces the boolean cuts create, up to 12 vertices. Fanned, their triangles fold across the notch and spill outside the outline, so sound booleans rendered as if they had failed. Triangle areas match the polygon areas to 5e-8 mm2.

* Changed `example_model_11_full.py` to merge `data/cantilevers_model.json`. It read `cantilever_model.json` - example 7's SINGLE column and quarter - so `full_model.json` was one quarter of the structure inside the complete shoring ring: 119 elements and 1 column. It is 362 elements and 4 columns.

* Changed the assembly page so only numbers are generated. The scripts used to write their own headings and sentences into `data/assembly/assembly_step*_points.md`, which made the wording unreachable from `docs/assembly.md`; each list is now wrapped in a snippet section (`--8<-- "...:holes"`) and the prose around it is ordinary markdown on the page.

* Changed the assembly previews to one colour. Every object is the same steel, and blue and pink now mean only "mark" - points to drill and footprints to measure - which is the whole subject of step 2 and appears in no other step. The jig boards of step 1 are that colour drawn at `d 0.4`, so the pocket under the piece can be seen.

* Changed step 2 to stake out the tower's four legs in the same session as the column bases, since the instrument is already up on the centre of the bay and a second set-up is a second chance to be wrong. Its shot is the whole floor rather than a 0.7 m close-up of `support_0` because of it. The props are not staked out: each clamps to a column and foots where that puts it.

* Changed `docs/fabrication.md`'s column row to `220 x 220 x 3350 incl. capitel`, which is what a 3500 bay gives.
