"""Assembly step 3 - screw the head plates onto the columns.

Second of the ``example_model_23_assembly_*`` series; see step 2 for how the
previews reach the docs.

A Power Base comes apart at its coupling nut. The half with the base plate
stays anchored in the slab from step 2; the Ø 106 x 12 head plate goes onto the
...
# ONLY the numbers are generated - the headings around them are prose in
# docs/assembly.md, which pulls each list in by section name
# (`...points.md:spec`), so the wording can change without re-running this.
lines = [
